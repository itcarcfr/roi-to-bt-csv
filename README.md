# Convertor ROI/CSV → CSV Banca Transilvania

Aplicație web simplă, fără server, care transformă fie un fișier `.ROI` (format de ordine de plată), fie un CSV vechi de plăți (21 de coloane, separator `;`, folosit anterior), într-un fișier CSV în formatul oficial acceptat de Banca Transilvania pentru încărcarea plăților multiple.

Formatul fișierului de intrare este detectat automat — nu trebuie ales manual.

## Cum se folosește

1. Deschide pagina (link GitHub Pages, mai jos).
2. Trage fișierul `.ROI` sau CSV-ul vechi în zona marcată, sau apasă și alege-l.
...
- **Suma (Amount)**: la fișierele `.ROI`, e calculată direct din cifre, presupunând RON întregi (fără bani/subunități), de exemplu `7130` → `7130.00` — verifică manual dacă vreo sumă are de fapt subunități. La CSV-ul vechi, suma vine deja completă din fișier (ex. `1840.00`) și e preluată ca atare.
- La CSV-ul vechi, **codul fiscal al beneficiarului** (coloana `CUI/CNP beneficiar`) e preluat direct, cu o excepție: valoarea `0` (folosită pentru persoane fizice, fără CUI) e tratată ca lipsă și lăsată goală.
